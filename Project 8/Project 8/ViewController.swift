...
//  Project 8
//
//  Created by Aleksandr Pavlov on 02.02.22.
//
...
class ViewController: UIViewController {
    ...
    override func loadView() {
        ...
        currentAnswer.placeholder = "Tap letters guess"
    ...
    @objc func submitTapped(sender: UIButton) {
        guard let answerText = currentAnswer.text else { return }
            ...
            if let solutionPosition = solutions.firstIndex(of: answerText) {
                ...
                if lettersButton.allSatisfy({$0.isHidden}) {
                    let ac = UIAlertController(title: "Well done!", message: "Are you ready for the next level?", preferredStyle: .alert)
                    ac.addAction(UIAlertAction(title: "Let's go!", style: .default, handler: levelUp))
                    present(ac, animated: true)
                }
            } else {
                let ac = UIAlertController(title: "WRONG", message: "Try one more time", preferredStyle: .alert)
